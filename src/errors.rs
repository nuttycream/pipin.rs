...
impl fmt::Display for GpioError {
    fn fmt(
        ...
        match self {
            ...
            GpioError::Set(pin) => write!(
                f,
                "Failed to set high - low {}",
                pin
            ),
            GpioError::Terminate => {
                write!(f, "Failed to terminate")
            }
            GpioError::SwitchDevice(dev) => write!(
                f,
            ...
            GpioError::PullDown(pin) => write!(
                f,
                "Failed to set {} to pull down",
                pin
            ),
            GpioError::PullUp(pin) => write!(
                f,
                "Failed to set {} to pull up",
                pin
            ),
            GpioError::HardwareDetection => {
                write!(
                    f,
                    "Failed to Detect Raspberry Pi Peripheral Address"
                )
            }
