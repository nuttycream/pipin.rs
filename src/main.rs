...
use actions::{
    add_action, delete_action, get_actions, start_actions,
    stop_actions, Action,
};
use axum::{
    extract::{
    ...
    match gpio.switch_device(device) {
        Ok(_) => {
            match config::load_conf() {
                ...
                Err(_) => {
                    println!(
                        "failed to update config new device"
                    );
                }
            }
            ...
            log_info(
                &appstate,
                format!("Switched to device {}", device),
            )
        }
        Err(e) => {
            println!("{e}");
            log_error(
                &appstate,
                format!("Failed to switch device: {e}"),
            )
        }
    }
}
...
async fn get_pins() -> Html<String> {
    let config = match config::load_conf() {
        ...
        Err(_) => {
            println!(
                "failed to load config for gpio pins frontend"
            );
            return Html(
                "<div>Error loading GPIO layout</div>"
                    .to_string(),
            );
        }
    };
...
async fn handle_socket(socket: WebSocket, state: AppState) {
    ...
    let mut recv_task = tokio::spawn(async move {
        while let Some(Ok(msg)) = receiver.next().await {
            if process_message(msg, state.clone())
                .is_break()
            {
                break;
            }
